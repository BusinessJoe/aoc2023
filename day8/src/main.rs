...
fn solution1(lines: impl IntoIterator<Item = impl AsRef<str>>) -> u32 {
    ...
    let dirs: Vec<Direction> = lines.next().unwrap().as_ref().as_bytes().iter().map(|&c| {
        if c == b'L' {
            Direction::Left
        } else {
            Direction::Right
    ...
    lines.skip(1).map(|line| parse_line(line.as_ref())).for_each(|(name, left, right)| {
...
fn merge_cycles(c1: &Cycle, c2: &Cycle) -> Cycle {
    ...
    let reps = period_len / c2.period_len;
    for r in 0..reps {
        for idx in &c2.periodic_zees {
            if c1.zees(head_len + idx + r * c2.period_len) {
                periodic_zees.insert(idx + r*c2.period_len);
            }
        }
    }
...
fn solution2(lines: impl IntoIterator<Item = impl AsRef<str>>) -> u64 {
    ...
    let final_cycle = names.iter().map(|name| {
        build_cycle(name.clone(), &map, &dirs)
    }).reduce(|c1, c2| {
        println!("reducing cycles with period length {} and {}", c1.period_len, c2.period_len);
        dbg!(&c1);
        dbg!(&c2);
        let c3 = merge_cycles(&c1, &c2);
        dbg!(&c3);
        if c3.periodic_zees.len() == 0 {
            panic!()
        }
        c3
    }).unwrap();

    println!("looking for answer in cycle with period length {}", final_cycle.period_len);
    (final_cycle.head_zees.len() + final_cycle.periodic_zees.iter().min().unwrap()).try_into().unwrap()
}
